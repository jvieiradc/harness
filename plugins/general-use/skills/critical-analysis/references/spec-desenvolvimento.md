...
Referência da área "especificação de projeto de desenvolvimento" da skill `critical-analysis`. Carregada só depois que a área foi inferida e confirmada.
...
**O que ela não produz:** a spec. Esta análise é a **base para a criação** da especificação — preserva raciocínio, alternativas descartadas e pendências, exatamente o que um documento final apaga. Quem for escrever a spec depois consome isto; não recebe pronto.
...
Se o usuário pedir explicitamente algo desta lista, isso é saída do modo de análise crítica: entregue a consolidação primeiro e trate o pedido como próxima tarefa, com a ferramenta adequada.
