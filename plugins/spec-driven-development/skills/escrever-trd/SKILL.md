...
> **Nota sobre o fluxo completo.** Esta skill faz parte de um fluxo maior de spec-driven
> development (PRD → TRD/ADR → SPEC/PLAN/TASKS → implementação). Neste harness, por enquanto,
> só `escrever-prd` e `escrever-trd` existem — `sdd-especificar` e o que viria depois dela ainda
> não foram trazidas pra cá. O **Modo Decision** (criação de ADR) e a manutenção do TRD
> funcionam de forma completa e útil por si só, sem depender de nada disso. Ver
> [`README.md`](./README.md) para o estado atual.
...
Quando o projeto ainda não tem código — tipicamente logo depois de uma sessão de
`critical-analysis` com a lente de especificação de projeto de desenvolvimento — não há
arquivo para analisar. A fonte é o **documento de origem** (o documento consolidado dessa
análise, ou outro documento que o usuário indicar): extraia dele Stack, Arquitetura,
Requisitos Não-Funcionais e Dependências Externas, pule o Passo 1, aplique o enriquecimento
externo (Passo 2) às dependências citadas e entreviste só as lacunas. Itens que o documento
marca como Verificados entram com a fonte e a data da consulta.
...
dados, interface entre fatias), mora no PLAN da fatia, gerado pelo `sdd-especificar` (quando essa
skill existir neste harness); se for reversível (qual util reusar, como fiar um endpoint), nem
precisa de registro. Se o pedido for de decisão local, avisar e sugerir o caminho certo em vez de
criar ADR.
...
- Gera SPEC, PLAN ou TASKS (papel do `sdd-especificar`, quando essa skill existir neste harness)
