...
> só `escrever-prd` e `escrever-trd` existem — as demais skills mencionadas abaixo
> (`sdd-especificar` e o que viria depois dela) ainda não foram trazidas pra cá. O PRD gerado
> aqui é completo e útil por si só; as menções a essas skills descrevem o desenho original do
> fluxo, não uma capacidade disponível agora. Ver [`README.md`](./README.md) para o estado atual.
...
partir do SPEC (derivado dele pelo `sdd-especificar`, quando essa skill existir neste harness).
O contexto técnico para implementar vem do TRD (carregado junto na implementação). O campo de
referências conecta PRDs entre si e a recursos externos quando necessário.
...
- No modo criação, o `status` inicial ao salvar é sempre `rascunho`. No modo edição, preservar o `status` atual do PRD — transições entre `pronto`, `em-progresso` e `concluido` são feitas por skills futuras de planejamento e execução (ainda não existentes neste harness — por ora, mude o campo manualmente quando for o caso)
